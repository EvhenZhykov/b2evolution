...
if( $action == 'req_login' && is_logged_in() && $forward_to !== NULL ) {
    header_redirect( $forward_to ); // Redirect to a requested URL if user is not logged-in:
}
...
switch( $action )
{
    ...
    case 'resetpassword':
        ...
        if ( is_email( $login ) ) {
            ...
            $not_activated_Ids = [];
            while ( ( $iterator_User = & $UserCache->get_next() ) )
    ...
    case 'changepwd':
        ...
        if ( !empty( $blog ) ) {
            ...
            if( $Blog ) {
                $changepwd_url = $Blog->get( 'userurl', [ 'url_suffix' => 'disp=pwdchange&reqID='.$reqID, 'glue' => '&' ] );
            }
        }
...
if ( $inskin && use_in_skin_login() ) {
    ...
    if ( ! empty( $Blog ) )	{
        ...
        if ( ( $action == 'req_activate_email' ) && !empty( $current_User ) ) {
            ...
            $redirect = $Blog->get( 'lostpasswordurl', [ 'glue' => '&' ] );
            ...
            $redirect = $Blog->get( 'loginurl', [ 'glue' => '&' ] );
        }
...
switch( $action )
{
    case 'lostpassword':
        ...
        $hidden_params = [ 'redirect_to' => url_rel_to_same_host( $redirect_to, get_htsrv_url( 'login' ) ) ];
        $wrap_width = '480px';
